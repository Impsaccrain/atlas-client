...
export function getDropRarity(mobRarity, highestPlayerRarity) {
    const minRarity = Math.max(0, mobRarity - 2);

    let rarity = minRarity;
    const myThreshold = Math.pow(threshold, mobRarity);

    for (let i = minRarity; i < mobRarity; i++) {
        if (Math.random() < myThreshold) rarity++;
    }

    return Math.min(rarity, highestPlayerRarity + 1);
}
