...
export async function loadUUID() {
    ...
    const data = await fetch(util.SERVER_URL + "/uuid/get?existing=" + existing).then(r => {
        if (!r.ok) throw new Error('Ran into an error fetching UUID data!');
        return r.json();
    });
...
export async function findLobbies() {
    ...
    if (!response.ok) throw new Error('Failed to fetch lobby list!');
...
new ChatMessage(1, "Atlas servers will save your progress until the server resets!", "#00FF00");
...
export class ClientSocket extends WebSocket {
    ...
    onMessage(event) {
        ...
        switch (reader.getUint8()) {
            ...
            case CLIENT_BOUND.PONG:
                ...
                break;
            case CLIENT_BOUND.PERMA_UUID:
                const uuid = reader.getUint16();
                localStorage.setItem('permaUUID', uuid);
                break;
    ...
    recover() {
        this.talk(SERVER_BOUND.PERMA_UUID);
    }
    ...
    talk(type, data) {
        ...
        switch (type) {
            case SERVER_BOUND.VERIFY:
                ...
                writer.setStringUTF8('ATLAS1.1'); // Atlas verification and version
                break;
            case SERVER_BOUND.PERMA_UUID:
                const permaUUID = parseInt(localStorage.getItem('permaUUID'));
                if (!Number.isNaN(permaUUID)) writer.setUint16(permaUUID);
                else writer.setUint16(0);
                break;
...
export async function loadAssets(lobbyID) {
    ...
    state.mobHealthScale = decoded.mobHealthScale;
    state.mobDamageScale = decoded.mobDamageScale;
