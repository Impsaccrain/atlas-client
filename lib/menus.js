...
for (let i = 0; i < 5; i++) {
    ...
    image.src = "https://impsaccrain.github.io/atlas-client/assets/" + child.id + ".svg";
...
export async function loadAndRenderChangelogs() {
    ...
    try {
        const data = await (await fetch("https://impsaccrain.github.io/atlas-client/assets/changelog.md")).text();
    ...
    if (localStorage.getItem("latestChangelog") !== latestChangelog) {
        ...
        const img = new Image();
        img.src = "https://impsaccrain.github.io/atlas-client/assets/alert.svg";
