...
export class MobTier {
    static HEALTH_SCALE = 3.95;
    static DAMAGE_SCALE = 3.15;
    static SIZE_SCALE = 1.1;
...
export class PetalConfig {
    ...
    setBodyDamage(bodyDamage) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].bodyDamage = bodyDamage * Math.pow(PetalTier.DAMAGE_SCALE, i);
        }
    ...
    setSelfDamage(selfDamage) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].selfDamage = selfDamage * Math.pow(PetalTier.DAMAGE_SCALE, i);
        }
    ...
    setPetHeal(petHeal) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].petHeal = (petHeal / 22.5) * Math.pow(MobTier.HEALTH_SCALE, i);
        }
    ...
    setDamageReduction(damageReduction) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].damageReduction = damageReduction * Math.pow(1.07, i);
        }
    ...
    setSpawnable(index, rarity, timer) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].spawnable = {
                ...
                timer: 22.5 * (timer instanceof Array ? (timer[i] ?? timer[timer.length - 1]) : timer)
            };
        }
    ...
    setExtraVision(extraVision) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].extraVision = extraVision * Math.pow(1.1, i);
        }
    ...
    setDeathDefying(healthRegain) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].deathDefying = Math.min(1, healthRegain * Math.pow(1.1883, i));
        }
...
export class MobConfig {
    ...
    setProjectile(projectile = {}) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].projectile = {
                ...
                range: (projectile.range ?? 50) * Math.pow(MobTier.SIZE_SCALE * .9, i),
    ...
    addDrop(index, chance = 1, minRarity = 0) {
        if ((index < 0 || index > 255) && index !== 'wormhole.any') {
...
export function encodePetalConfig(config) {
    ...
    if (config.tiers[0].selfDamage > 0) {
        output[flagsIndex] |= 0x20000000;
    };

    if (config.tiers[0].petHeal > 0) {
        output[flagsIndex] |= 0x40000000;
    };

    if (config.tiers[0].bodyDamage > 0) {
    ...
    output.push(...config.tiers.flatMap((tier, tierID) => {
        ...
        if (output[flagsIndex] & 0x20000000) {
            tierOutput.push(tier.selfDamage);
        };

        if (output[flagsIndex] & 0x40000000) {
            tierOutput.push(tier.petHeal);
        };

        if (output[flagsIndex] & 0x80000000) {
            tierOutput.push(tier.bodyDamage);
        };
...
export function decodePetalConfig(data, nTiers) {
    ...
    for (let i = 0; i < nTiers; i++) {
        ...
        if (flags & 0x20000000) {
            tier.selfDamage = data.shift();
        }

        if (flags & 0x40000000) {
            tier.petHeal = data.shift();
        }

        if (flags & 0x80000000) {
            tier.bodyDamage = data.shift();
        }
...
function encodeMobConfig(config) {
    return [config.id, config.name, +config.isSystem, config.health, config.damage];
}
...
export async function loadTerrains() {
    const response = await fetch("https://impsaccrain.github.io/atlas-client/assets/terrains.json");
    if (!response.ok) throw new Error('Failed to load default terrains!');
