...
function createPetalTooltip(index, rarityIndex) {
    ...
    if (tier.spawnable !== undefined) {
        ...
        text("- Health: " + formatLargeNumber(+(config.health * Math.pow(state.mobHealthScale, tier.spawnable.rarity)).toFixed(2)), 20, newY + 17.5, 15, colors.common, ctx);
        text("- Damage: " + formatLargeNumber(+(config.damage * Math.pow(state.mobDamageScale, tier.spawnable.rarity)).toFixed(2)), 20, newY + 17.5 * 2, 15, mixColors(colors.legendary, "#FFFFFF", .2), ctx);
