...
function drawWormhole(wormholePulseNumber, hit = false, ctx = _ctx) {
    const fillCol = mixColors("#000000", "#FF0000", hit * .5);
    const strokeCol = mixColors(fillCol, "#FFFFFF", wormholePulseNumber);
...
export function drawMob(id, index, rarity, hit = false, ctx = _ctx, attack = false, friend = false, rot = 0, extra = undefined, wormholePulseNumber = 0) {
    switch (index) {
        case 0:
        ...
        case 73:
            ...
            break;
        case 74:
            drawWormhole(wormholePulseNumber, hit, ctx);
            break;
    }
}
...
function createPetalTooltip(index, rarityIndex) {
    ...
    if (tier.spawnable !== undefined) {
        ...
        text("- Health: " + formatLargeNumber(+(config.health * Math.pow(MobTier.HEALTH_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY, 15, colors.common, ctx);
        newY += 17.5;

        text("- Damage: " + formatLargeNumber(+(config.damage * Math.pow(MobTier.DAMAGE_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY, 15, mixColors(colors.legendary, "#FFFFFF", .2), ctx);
        newY += 17.5;
    }
