...
export class PetalConfig {
    ...
    setPetHeal(petHeal) {
        for (let i = 0; i < this.tiers.length; i++) {
            this.tiers[i].petHeal = (petHeal / 22.5) * Math.pow(PetalTier.HEALTH_SCALE, i);
        }
...
export function encodeEverything(tiers, petalConfigs, mobConfigs) {
    ...
    output.push(MobTier.HEALTH_SCALE, MobTier.DAMAGE_SCALE);
...
export function decodeEverything(data) {
    ...
    const scales = [data.shift(), data.shift()];

    return {
        tiers: tiers,
        petalConfigs: petals,
        mobConfigs: mobs,
        mobHealthScale: scales[0],
        mobDamageScale: scales[1]
    };
}
