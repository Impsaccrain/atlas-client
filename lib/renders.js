...
function drawWormhole(hit = false, ctx = _ctx) {
    ...
    const strokeCol = mixColors(fillCol, "#FFFFFF", 0.1);
...
function createPetalTooltip(index, rarityIndex) {
    ...
    if (tier.spawnable !== undefined) {
        ...
        text("- Health: " + formatLargeNumber(+(config.health * Math.pow(MobTier.HEALTH_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY + 17.5, 15, colors.common, ctx);
        text("- Damage: " + formatLargeNumber(+(config.damage * Math.pow(MobTier.DAMAGE_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY + 17.5 * 2, 15, mixColors(colors.legendary, "#FFFFFF", .2), ctx);
