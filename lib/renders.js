...
import { Drawing, MobTier, PetalConfig } from "./protocol.js";
...
function concavePolygon(ctx, sides, radius, rotation, concavity) {
    const step = TAU / sides;
    const innerR = radius * (1 - concavity);

    ctx.beginPath();
    for (let i = 0; i < sides; i++) {
        const a0 = rotation + i * step;
        const a1 = rotation + (i + 1) * step;
        ...
        const am = (a0 + a1) / 2;
        const cx = Math.cos(am) * innerR;
        const cy = Math.sin(am) * innerR;
...
function drawBloodStinger(rarity, ctx = _ctx) {
    const count = lightCount[rarity];
    setStyle(ctx, colors.fireAnt, count > 0 ? .1 : .2);
...
function drawAntEgg(ctx = _ctx, color, count) {
    setStyle(ctx, color, .2);
...
export function drawCutter(ctx = _ctx, hit = false) {
    ...
    ctx.beginPath()
    concavePolygon(ctx, 6, 1.25, Math.PI / 6, 0.35);
...
function drawHornetEgg(ctx = _ctx, count) {
...
export function drawPetal(index, hit = false, ctx = _ctx, id = 0) {
    ...
    switch (index) {
        ...
        case 73: // Diep Pentagon
            ...
            break;
        case 74: // Termite Eggs
            basicPetal(ctx, hit, colors.termite);
            break;
        case 76: // Fire Ant Eggs
            basicPetal(ctx, hit, colors.fireAnt);
            break;
        case 77: // Blood Stinger
            trianglePetal(ctx, hit, colors.fireAnt);
            break;
        case 78: // Cutter
            drawCutter(ctx, hit);
            break;
...
export function drawUIPetal(index, rarity, ctx = _ctx) {
    switch (index) {
        ...
        case 37: // Powder
            ...
            break;
        case 38: // Ant Egg
            drawAntEgg(ctx, colors.peach, rarity > 12 ? 5 : 4);
            break;
        case 45: // Dandelion
            drawDandyIcon(ctx);
            break;
        ...
        case 63: // Lantern
            ...
            break;
        case 67: // Hornet Egg
            drawHornetEgg(ctx, rarity > 12 ? 3 : 2);
            break;
        case 68: // Candy
            ctx.save();
            ctx.scale(1.05, 1.05);
            ...
            break;
        case 74: // Termite Egg
            drawAntEgg(ctx, colors.termite, rarity > 12 ? 3 : 2);
            break;
        case 75: // Vyior
            break;
        case 76: // Fire Ant Egg
            drawAntEgg(ctx, colors.fireAnt, rarity > 12 ? 4 : 3);
            break;
        case 77: // Blood Stinger
            drawBloodStinger(rarity, ctx);
            break;
        default:
...
function drawWormhole(pulse, hit = false, ctx = _ctx) {
    const fillCol = mixColors("#000000", "#FF0000", hit * .5);
    const strokeCol = mixColors(fillCol, "#FFFFFF", pulse / 4 + 0.1);
...
function drawSoldierFireAnt(id, attack, hit, ctx = _ctx, friend = false) {
    drawSoldierAntT(id, friend ? colors.ladybugRed : colors.fireAnt, attack, hit, ctx);
}
...
function drawSoldierTermite(id, attk = false, hit = false, ctx = _ctx, friend = false) {
    setStyle(ctx, mixColors(friend ? colors.playerYellow : colors.termite, "#FF0000", hit * .5), .4);
...
export function drawMob(id, index, rarity, hit = false, ctx = _ctx, attack = false, friend = false, rot = 0, extra = undefined, pulse = 0) {
    switch (index) {
        case 0:
        ...
        case 21:
            ...
            break;
        case 22:
            drawSoldierFireAnt(id, attack, hit, ctx, friend);
            break;
        case 23:
        ...
        case 26:
            ...
            break;
        case 27:
            drawSoldierTermite(id, attack, hit, ctx, friend);
            break;
        case 28:
...
function createPetalTooltip(index, rarityIndex) {
    ...
    if (tier.spawnable !== undefined) {
        const config = state.mobConfigs[tier.spawnable.index];

        let x = 10 + text("Spawns: ", 10, newY, 15, colors.bubbleGrey, ctx);
        x += text(config.name, x, newY, 15, colors.peaGreen, ctx);
        x += text(" (", x, newY, 15, colors.bubbleGrey, ctx);
        x += text(state.tiers[tier.spawnable.rarity].name, x, newY, 15, state.tiers[tier.spawnable.rarity].color, ctx);
        text(")", x, newY, 15, colors.bubbleGrey, ctx);
        newY += 17.5;

        text("- Mob Health: " + formatLargeNumber(+(config.health * Math.pow(MobTier.HEALTH_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY, 15, colors.common, ctx);
        newY += 17.5;

        text("- Mob Damage: " + formatLargeNumber(+(config.damage * Math.pow(MobTier.DAMAGE_SCALE, tier.spawnable.rarity)).toFixed(2)), 20, newY, 15, mixColors(colors.legendary, "#FFFFFF", .2), ctx);
        newY += 17.5;
    }
    ...
    if (tier.selfDamage > 0) {
        text("Self Damage: " + formatLargeNumber(+tier.selfDamage.toFixed(2)), 10, newY, 15, colors.legendary, ctx);
        newY += 17.5;
    };

    if (tier.petHeal > 0) {
        text("Pet Heal: " + formatLargeNumber(+tier.petHeal.toFixed(2)), 10, newY, 15, colors.rosePink, ctx);
        newY += 17.5;
    };

    if (tier.bodyDamage > 0) {
        text("Extra Body Damage: " + formatLargeNumber(+tier.bodyDamage.toFixed(2)), 10, newY, 15, colors.rare, ctx);
        newY += 17.5;
    };
